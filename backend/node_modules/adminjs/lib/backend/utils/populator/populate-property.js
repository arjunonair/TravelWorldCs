...
async function populateProperty(records, property, context) {
  ...
  const referenceRecords = await referencedResource.findMany(uniqueExternalIds, context); // even if record has value for this reference - it might not have the referenced record itself
